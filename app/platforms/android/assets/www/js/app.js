...
var app = angular.module('PHR', [
	...
	.config(['$routeProvider', '$stateProvider', '$urlRouterProvider', '$locationProvider', '$httpProvider', 'API', function ($routeProvider, $stateProvider, $urlRouterProvider, $locationProvider, $httpProvider, API) {
 ...
 */
	...
	.controller('BasicInformationCtrl', ['$scope', '$state', '$filter', 'Auth', 'RELATIONSHIPS', function($scope, $state, $filter, Auth, RELATIONSHIPS){
		...
		$scope.submitForm = function() {
			$scope.user.relationship = $scope.relationships.indexOf($scope.user.relationship);
			$scope.user.gender = $scope.genders.indexOf($scope.user.gender);
			if($scope.is_add_account){
				Auth.setTempAccount(
					$scope.user,
					1,
					function() {
						$state.go('user.add-account.personal-info');
					}
				);
				...
				Auth.update(
					$scope.user,
					function() {
						$state.go('user.signup.personal-info');
					},
					function(error){
						console.log(error);
					},
	...
	.controller('ItemCreateCtrl', ['$scope', '$state', '$location', 'ApiService', function($scope, $state, $location, ApiService){
		...
		$scope.submitForm = function() {
			ApiService.create(
				...
				$scope.collection,
				function(data){
					$location.path($scope.itemData.type + '/' + $scope.itemData.shortcutSingle + '/' + data['id'] + '/edit');
				},
				function(err){"ERR", console.log(err) });
		};
				...
				if(isNotSurgical){
					if(isNew){
						...
						if($scope.itemData.shortcut != 'conditions'){
							$scope.conditions.relationship = $scope.relationships[conditions.relationship-1];
						}
					}
				}
			},
		...
		$scope.submitForm = function() {
			if(isNotSurgical){
				var conditions = {
					...
					'relationship': (typeof $scope.conditions.relationship === 'undefined')?Auth.user.relationship:$scope.relationships.indexOf($scope.conditions.relationship)+1
				};
				console.log(conditions.relationship)
 ...
 * Docs
 */
  	.controller('DocumentsUploadCrtl', ['$scope', '$route', '$stateParams', 'ApiService', function($scope, $route, $stateParams, ApiService){

  	}])

	.controller('DocumentsListCrtl', ['$scope', '$route', '$stateParams', 'ApiService', function($scope, $route, $stateParams, ApiService){
		ApiService.list('documents', function(documents) {
...
appServices
	.factory('ApiService', ['$http', '$interval', 'API', 'Auth','localStorageService', 'Notification', function ($http, $interval, API, Auth, localStorageService, Notification) {
		...
		return {
			...
			create: function(object_name, data, callback, error_callback) {
	...
	.factory('UserService', ['$rootScope', '$http', '$timeout', '$location', '$state', 'localStorageService', 'API', 'Notification', 'Overlay', function ($rootScope, $http, $timeout, $location, $state, localStorageService, API, Notification, Overlay) {
		return {
			...
			createAccount: function(user, callback, error_callback) {
				...
				$http.post( API['URL_PATIENTS'], user, {headers: headers})
					.success(function (response) {
						...
						Notification.show('Account created successfully', 'success', 0);
						return callback(response);
					}).error(function (error) {
			...
			healthProfileQuestions: function(user, callback, error_callback){
				...
				$http.post( API['URL_PATIENTS'] + '/' +  user.id + '/healthprofileqna', data, {headers: headers})
					.success(function () {
						var msg = 'Onboarding health questions API error';
						console.log(msg);
						return callback();
					}).error(function () {
